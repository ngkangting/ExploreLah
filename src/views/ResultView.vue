    ...
    <div v-if="isLoading">
      <div class="row d-flex py-5 px-4 px-md-5">
            ...
            <div class="card-body">
              <div class="row mb-3">
                ...
                <div class="col-4 d-none d-lg-flex flex-column justify-content-between">
                  <div class="row">
                    <div class="col-12">
                      <div class="w-100 rounded skeleton" style="height: 165px"></div>
                    </div>
                  </div>
                  <div class="row">
                    <div class="col-12">
              ...
              <div class="row">
                <div class="col-12 col-lg-6 rounded-3 px-3 py-3">
                  <div class="w-100 rounded skeleton" style="height: 500px"></div>
                </div>

                <div class="column col-12 col-lg-6 rounded-3 px-3 py-3">
                  <!-- Header -->
                  <div class="skeleton skeleton-text rounded pb-1 w-50" style="height: 35px"></div>
                  <!-- Tags -->
                  <div class="d-flex">
                    <div class="skeleton skeleton-text rounded me-2 w-25" style="height: 18px"></div>
                    <div class="skeleton skeleton-text rounded mx-2 w-25" style="height: 18px"></div>
                    <div class="skeleton skeleton-text rounded mx-2 w-25" style="height: 18px"></div>
                    <div class="skeleton skeleton-text rounded mx-2 w-25" style="height: 18px"></div>
                  </div>
                  ...
                  <div>
                    <div class="d-flex align-items-center my-2">
                      <div class="skeleton rounded-circle skeleton-text my-auto" style="height: 30px; width: 30px">
                      </div>
                      <div class="skeleton skeleton-text rounded ms-2 w-50" style="height: 20px"></div>
                    </div>
        ...
        <div class="col-12 col-md-6 col-lg-4 order-1 order-md-2 position-relative">
          <div class="skeleton skeleton-text rounded mx-auto my-2 mb-4 w-50" style="height: 35px"></div>

          <div class="nav nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
            <PillTabSkeleton v-for="(activity, index) in 3" :key="index" :index="index" :isLast="index == 2" />
          </div>
        </div>
      </div>
    </div>
...
" class="w-100 rounded" height="350" />
                  ...
                  <div v-else class="w-100 rounded bg-light position-relative" style="height: 350px">
                    <span class="fs-4 position-absolute top-50 start-50 translate-middle text-secondary text-center">No
                      Image Found</span>
                  </div>
                </div>
                <div class="col-4 d-none d-lg-flex flex-column justify-content-between">
...
" class="w-100 rounded" height="165" />
                      ...
                      <div v-else class="w-100 rounded bg-light position-relative" style="height: 165px">
                        <span class="position-absolute top-50 start-50 translate-middle text-secondary text-center">No
                          Image Found</span>
                      </div>
                    </div>
                  </div>
...
" class="w-100 rounded" height="165" />
                ...
                <div class="column col-12 col-lg-6 rounded-3 px-3 py-3">
                  ...
                  <div class="d-flex flex-wrap mb-4">
                    <span v-for="(tag, index) in currentDayData[idx].tags" :key="index"
                      class="tag bg-grey rounded-2 py-1 px-2 me-2 my-1 text-nowrap">{{ tag }}</span>
                  </div>
                  ...
                  <table>
                    ...
                    <tr v-if="currentDayData[idx].status.toLowerCase() == 'clear'">
                      <td>
                        <i class="bg-grey bi bi-sun-fill rounded-circle p-2 text-dark-blue"></i>
                      </td>
                    ...
                    <tr v-if="currentDayData[idx].status.toLowerCase() == 'rain'">
                      <td>
                        <i class="bg-grey bi bi-cloud-drizzle-fill rounded-circle p-2 text-dark-blue"></i>
                      </td>
                      <td style="padding-left: 10px">
                        Forecast: {{ currentDayData[idx].status }}
                      </td>
                    ...
                    ">
                      <td>
                        <i class="bg-grey bi bi-cloud-fill rounded-circle p-2 text-dark-blue"></i>
                      </td>
                      <td style="padding-left: 10px">
                        Forecast: {{ currentDayData[idx].status }}
                      </td>
                    ...
                    <tr>
                      <td>
                        <i class="bg-grey bi bi-clock-fill rounded-circle p-2 text-dark-blue"></i>
                      </td>
                      <td v-if="
                        'opening_hours' in
                        currentDayDetails[idx][currentDayData[idx].placeID]
                    ...
                    ">
                      ...
                      <td>
                        <ul style="list-style: none; padding-left: 10px">
                          <li v-for="(day, index) in currentDayDetails[idx][
                            currentDayData[idx].placeID
                          ].result.opening_hours.weekday_text" :key="index">
                            {{ day }}
                          </li>
                        </ul>
                      </td>
          ...
          <div class="position-absolute bottom-0 end-0 p-4 d-block d-md-none">
            <router-link to="/foodview" class="text-decoration-none text-secondary">
              <span class="ps-1"> See Food Recommendations </span>
              <i class="bi-chevron-right text-secondary" style="font-size: 1rem"></i>
            </router-link>
          </div>
        </div>

        <!-- Locations -->
        <div class="col-12 col-md-6 col-lg-4 order-1 order-md-2 position-relative mb-4 mb-md-0">
          <h4 class="text-center my-2 mb-4">
            Day {{ currDay }} of {{ this.itineraryStore.itineraryList.length }}
          </h4>

          <div class="nav nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
            <PillTab v-for="(activity, index) in currentDayData" :key="index" :index="index" :isSelected="idx == index"
              :isLast="index == currentDayData.length - 1" :place="activity.name" :type="activity.tags"
              :arrivalTime="activity.arriveTime" :departureTime="activity.endTime" :duration="activity.dur"
